...
export default function ResumePreview({label = 'View résumé', className = ''}: {label?: string; className?: string}) {
  const heading = useRef<HTMLHeadingElement>(null);
  return <Dialog>
    <DialogTrigger asChild><button className={`text-link resume-trigger ${className}`}>{label} <ArrowUpRight size={18}/></button></DialogTrigger>
